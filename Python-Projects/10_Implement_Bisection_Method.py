...
def square_root_bisection(num,tolerance=1e-7,max_iterations=100):
    if num<0:
    ...
    # For numbers < 1, the root is between the number and 1 (e.g., sqrt(0.001) is ~0.0316)
    low = 0
    high = max(1,num)
    root = None
    ...
    for i in range(max_iterations):
        ...
        # Check if the current interval is smaller than the tolerance
        # Or if we hit the square perfectly
